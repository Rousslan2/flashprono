...
// Pronostics CRUD minimal
router.get("/pronostics", async (_req, res, next) => {
  try { const items = await Pronostic.find().sort({ createdAt: -1 }); res.json(items); } catch (e) { next(e); }
});
router.post("/pronostics", async (req, res, next) => {
  try { if (!req.user.isAdmin) return res.status(403).json({ message: "Accès refusé" });
        const prono = await Pronostic.create(req.body); res.json(prono); } catch (e) { next(e); }
});
